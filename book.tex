...
There are 4 number sets, insane $\mathbb{I}$, less insane $\mathbb{L}$, sane
$\mathbb{S}$ and more sane $\mathbb{M}$. We use insane and less insane
numbers only to define sane and more sane numbers. Then we never use those
insane numbers any more.
...
\emph{insane} and use it briefly only to define two numbers after which we
immediately jump back to our saner reality.

Why do we need them? Only to define two numbers as follows:
\begin{itemize}
\item I define that there is a function $f : \mathbb{I} \rightarrow \mathbb{I}$ that for
any mass $m \in \mathbb{I}$, it returns the maximum number ever representable by $m$.
Then I define $\maxnum = \maxn(\unimass)$, which is the largest number ever
representable by the material universe. Self explanatory, no need for excess
rigor.
\item Likewise, $\minnum = \minm(\unimass)$ is the smallest number that is
greater than zero ever representable by the material universe.
\end{itemize}

That's it. We don't need $\mathbb{I}$ for anything else (other than
operations that apply on them, like addition, multiplication, etc)

\section{Less insane numbers ($\mathbb{L}$)}
$\mathbb{L}$ is essentially what the powerful community of mathematicians calls
the set of natural numbers.


\section{Sane numbers ($\mathbb{S}$)}
Welcome back to sanity:
\[
    \mathbb{S} = \{i : i \in \mathbb{L}, -\maxnum \le i \le \maxnum\}
\]
where the impossible becomes possible:
\begin{itemize}
    \item Yes there is a maximum number: $\maxnum$.
    \item Yes the cardinality of $\mathbb{S}$ is known: $|\mathbb{S}| = 1 +
    2\maxnum$.
\end{itemize}
...
\[
    \mathbb{M} = \{i : i \in \mathbb{L}, -\maxnum \le i\minnum \le \maxnum\}
\]
